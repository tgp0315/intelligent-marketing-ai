 ...
 */
export function timeFormat(timestamp: string | number, mustIncludeTime: boolean = false): string {
  const currentDate = new Date()
  const srcDate = new Date(parseInt(timestamp as string))
  ...
  if (currentYear === srcYear) {
    ...
    const deltaTime = currentTimestamp - Number(srcTimestamp)
